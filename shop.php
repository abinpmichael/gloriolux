...
<div class="container" style="padding-top: 60px; padding-bottom: 50px;">
    ...
    <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 1rem; margin-bottom: 3rem;">
        <a href="shop" class="btn <?php echo !$category_id ? 'btn-primary' : 'btn-outline'; ?>" style="<?php echo !$category_id ? '' : 'color: var(--primary-color); border-color: var(--primary-color);'; ?>">All</a>
        <?php foreach($categories as $cat): ?>
            <a href="shop?category=<?php echo $cat['id']; ?>" class="btn <?php echo $category_id == $cat['id'] ? 'btn-primary' : 'btn-outline'; ?>" style="<?php echo $category_id == $cat['id'] ? '' : 'color: var(--primary-color); border-color: var(--primary-color);'; ?>"><?php echo htmlspecialchars($cat['name']); ?></a>
        <?php endforeach; ?>
    </div>
    ...
    <div class="product-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 2.5rem;">
        <?php foreach($products as $product): ?>
            <div class="product-card">
                <a href="<?= BASE_URL ?>product?id=<?php echo $product['id']; ?>">
                    <div class="product-img-wrap">
                        <img src="<?= BASE_URL ?><?php echo htmlspecialchars($product['image_url']); ?>" alt="Gloriolux <?php echo htmlspecialchars($product['name']); ?> Luxury Soy Candle Hand-Poured in Calgary" class="product-img">
                        <div class="product-overlay">
                            <form action="<?= BASE_URL ?>cart_add" method="POST">
